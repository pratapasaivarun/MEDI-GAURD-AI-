...
def _render_login() -> None:
    st.markdown('<div class="auth-shell"><div class="eyebrow">SECURE LOCAL WORKSPACE</div><h1>Medi Gaurd AI</h1><p>Medical bill and insurance claim verification</p></div>', unsafe_allow_html=True)
    login_tab, register_tab, reset_tab = st.tabs(["Sign in", "Create claimant account", "Reset password"])
    with login_tab:
        with st.form("login"):
        ...
        if submitted:
            if p1 != p2:
                ...
                try:
                    ...
                    st.error(str(exc))
    with reset_tab:
        st.caption("For privacy, the response does not reveal whether an email exists.")
        with st.form("reset_request"):
            email = st.text_input("Account email", key="reset_email")
            submitted = st.form_submit_button("Generate reset link", use_container_width=True)
        if submitted:
            ...
            if request["reset_token"]:
                st.code(f"{APP_BASE_URL}/?reset_token={request['reset_token']}")


def _render_dashboard(user: dict) -> None:
    st.title("Dashboard")
    st.caption("A quick view of claims, review workload, and system readiness.")
    claims = user_claims(user["user_id"], user)
    c1, c2, c3 = st.columns(3)
    c1.metric("Your claims", len(claims))
    c2.metric("Needs review", sum(1 for c in claims if c["status"] == "manual_review"))
    c3.metric("Role", user["role"].title())
    st.subheader("Recent claims")
    if not claims:
        st.info("No claims yet. Use Register claim to start a new verification.")
        return
    for claim in claims[:8]:
        with st.container(border=True):
            left, mid, right = st.columns([2, 4, 2])
            left.markdown(f"**{claim['claim_number']}**")
            mid.write(f"{claim['patient_name']} · {claim['hospital_name'] or 'Provider not entered'}")
            right.write(_status_badge(claim['status']))
            if st.button("Open claim", key=f"open_{claim['claim_id']}"):
    ...
    if user["role"] in {"reviewer", "admin"}:
        ...
        if queue:
            ...
            st.dataframe([{"Claim": c["claim_number"], "Patient": c["patient_name"], "Status": _status_badge(c["status"]), "Priority": c["review_priority"]} for c in queue], use_container_width=True, hide_index=True)


def _render_register(user: dict) -> None:
    st.title("Register claim")
    st.caption("Create the claim shell first. Documents and verification are completed in the next step.")
    with st.form("claim_form", clear_on_submit=False):
        ...
        with b:
            policy = st.text_input("Policy number", placeholder="POL-DEMO-2026-001")
            incident_date = st.date_input("Admission or service date")
            st.info("Use the policy number exactly as shown on the bill and policy document.")
        submitted = st.form_submit_button("Create claim and continue", type="primary", use_container_width=True)
    if submitted:
        if not claim_number.strip() or not patient.strip():
            ...
            st.session_state.page = "Claim review"
            st.success("Claim created. Continue to document upload.")
            st.rerun()


def _render_claim_review(user: dict) -> None:
    st.title("Claim review")
    st.caption("Complete the workflow in four short steps: select, upload, extract, then adjudicate.")
    claims = user_claims(user["user_id"], user)
    if not claims:
        st.info("Create a claim first from Register claim.")
        return
    ...
    selected = st.selectbox("1. Select claim", labels, index=default)
    claim_id = options[selected]
    st.session_state.active_claim = claim_id
    selected_claim = next(c for c in claims if c["claim_id"] == claim_id)
    with st.container(border=True):
        st.markdown("**2. Upload documents**")
        policy_files = st.file_uploader("Insurance policy", type=["pdf", "png", "jpg", "jpeg"], accept_multiple_files=True, key=f"pol_{claim_id}")
        bill_files = st.file_uploader("Medical bill", type=["pdf", "png", "jpg", "jpeg"], accept_multiple_files=True, key=f"bill_{claim_id}")
        if st.button("Save documents", type="primary", use_container_width=True):
            ...
            if not files:
                st.warning("Upload at least one policy or medical bill.")
            else:
                try:
                    for uploaded, kind in files:
                        save_document(claim_id, user, uploaded, kind)
                    st.success(f"Saved {len(files)} document(s).")
                    st.rerun()
                except (ValueError, PermissionError) as exc:
                    st.error(str(exc))
    docs = claim_documents(claim_id, user)
    if not docs:
        st.info("Your saved documents will appear here.")
        return
    st.markdown("**Saved documents**")
    st.dataframe([{"File": d["original_name"], "Type": d["document_type"].replace('_', ' ').title(), "Status": d["processing_status"]} for d in docs], use_container_width=True, hide_index=True)
    if st.button("3. Extract and normalize", type="primary", use_container_width=True):
        with st.status("Extracting and normalizing documents...", expanded=True) as status:
            try:
    ...
    if not normalized:
        st.info("After extraction, the normalized claim and analysis controls will appear here.")
        return
    missing = normalized.get("missing_fields", [])
    review = normalized.get("review_fields", [])
    if missing or review:
        st.warning("Review required: " + ", ".join(missing + review))
    fields = []
    for key in ("patient_name", "hospital_name", "policy_number", "claim_number", "admission_date", "discharge_date", "diagnosis", "total_amount"):
        ...
        if item:
            fields.append({"Field": key.replace('_', ' ').title(), "Value": item["value"], "Confidence": round(float(item["confidence"]), 1), "Review": "Yes" if item["needs_review"] else "No"})
    with st.expander("Extracted claim fields", expanded=True):
        st.dataframe(fields, use_container_width=True, hide_index=True)
    if normalized.get("line_items"):
        with st.expander(f"Bill line items ({len(normalized['line_items'])})", expanded=True):
            st.dataframe([{k: item.get(k) for k in ("description", "amount", "category", "confidence", "needs_review")} for item in normalized["line_items"]], use_container_width=True, hide_index=True)
    with st.expander("Reviewer corrections and trusted facts"):
        st.caption("Only Reviewer and Admin roles can save corrections or trust claimant assertions.")
        network = st.selectbox("Network status", ["unknown", "in_network", "out_of_network"], key=f"network_{claim_id}")
        preauth = st.selectbox("Pre-authorization", ["unknown", "confirmed", "not_confirmed"], key=f"preauth_{claim_id}")
        trusted = st.checkbox("Authorized reviewer verified these facts", disabled=user["role"] not in {"reviewer", "admin"}, key=f"trusted_{claim_id}")
        if st.button("Save review inputs", key=f"save_context_{claim_id}"):
            try:
                save_adjudication_context(claim_id, user, {"network_status": network, "preauthorization_status": preauth, "trusted": trusted})
                st.success("Review inputs saved.")
            except (ValueError, PermissionError) as exc:
                st.error(str(exc))
    st.divider()
    st.markdown("**4. Analyze claim**")
    ...
    if st.button("Run deterministic coverage rules", type="primary", use_container_width=True):
        with st.status("Evaluating policy rules...", expanded=True) as status:
            try:
                rules = evaluate_saved_claim(claim_id, user["user_id"], normalized)
                st.session_state[f"rules_{claim_id}"] = rules
                status.update(label=f"Rules complete: {_status_badge(rules['status'])}", state="complete")
            except Exception as exc:
                status.update(label="Rules failed", state="error")
                st.error(str(exc))
    rules = st.session_state.get(f"rules_{claim_id}")
    if not rules:
        st.info("Run deterministic rules before starting the agents.")
        return
    st.subheader(f"Deterministic result: {_status_badge(rules['status'])}")
    cols = st.columns(4)
    cols[0].metric("Covered", f"INR {rules.get('covered_amount', 0):,.2f}")
    cols[1].metric("Deductible", f"INR {rules.get('deductible', 0):,.2f}")
    cols[2].metric("Copayment", f"INR {rules.get('copayment', 0):,.2f}")
    cols[3].metric("Payable", f"INR {rules.get('payable_amount', 0):,.2f}")
    if rules.get("warnings"): st.warning("; ".join(rules["warnings"]))
    st.dataframe(rules.get("results", []), use_container_width=True, hide_index=True)
    ...
    if st.button("Run Policy Agent + Decision Agent", type="primary", use_container_width=True):
        with st.status("Preparing policy evidence...", expanded=True) as status:
            started = time.perf_counter()
            try:
                workflow = run_agents_for_claim(claim_id, user["user_id"], normalized, rules, progress_callback=lambda msg: status.write(msg))
                workflow["elapsed_seconds"] = round(time.perf_counter() - started, 2)
                st.session_state[f"workflow_{claim_id}"] = workflow
                status.update(label=f"Agent workflow complete in {workflow['elapsed_seconds']}s", state="complete")
            except Exception as exc:
                status.update(label="Agent workflow failed", state="error")
                st.error(str(exc))
    if workflow:
        decision = workflow.get("decision", {})
        st.subheader(f"Agent recommendation: {_status_badge(decision.get('status', 'manual_review'))}")
        ...
        with st.expander("Policy evidence and agent details"):
            st.dataframe(workflow.get("policy_findings", {}).get("retrieved_evidence", []), use_container_width=True, hide_index=True)
            st.json({"decision": decision, "policy_findings": workflow.get("policy_findings", {})})
        if user["role"] in {"reviewer", "admin"}:
            with st.expander("Authorized reviewer sign-off", expanded=True):
                statuses = ["approved", "partially_approved", "rejected", "manual_review"]
                final = st.selectbox("Final determination", statuses, format_func=_status_badge, key=f"final_{claim_id}")
                comments = st.text_area("Reviewer comments", key=f"comments_{claim_id}")
                if st.button("Save reviewer decision", key=f"save_final_{claim_id}", type="primary"):
                    if not comments.strip(): st.warning("Reviewer comments are required.")
                    else:
                        save_reviewer_decision(claim_id, user["user_id"], decision.get("status", "manual_review"), final, comments)
                        st.success("Reviewer decision saved and audited.")
        st.caption("The result is decision support only. Final determination must be confirmed by an authorized reviewer.")


def _render_policy_management(user: dict) -> None:
    st.title("Policy management")
    st.caption("Ingest policy editions once, keep history, and activate the correct edition before analyzing claims.")
    if user["role"] not in {"reviewer", "admin"}:
        st.warning("Reviewer or Admin access is required to ingest and manage policy editions.")
        return
    with st.container(border=True):
        with st.form("policy_ingestion_form"):
            policy_file = st.file_uploader("Policy PDF or policy photo", type=["pdf", "png", "jpg", "jpeg"])
            a, b = st.columns(2)
            with a:
                policy_number = st.text_input("Policy number", placeholder="POL-DEMO-2026-001")
            ...
            submitted = st.form_submit_button("Ingest and activate edition", type="primary", use_container_width=True)
        if submitted:
            if not policy_file or not policy_number or not version_label:
                ...
                try:
                    ...
                    st.success(f"Active policy created with {result['chunks_indexed']} evidence chunks.")
                    st.rerun()
                except Exception as exc: st.error(f"Policy ingestion failed: {exc}")
    versions = policy_versions()
    if versions:
        st.subheader("Policy editions")
        rows = []
        for version in versions:
            try: terms = json.loads(version["policy_terms_json"] or "{}")
            except (TypeError, json.JSONDecodeError): terms = {}
            rows.append({"Policy": version["policy_number"], "Version": version["version_label"], "Insurer": version["insurer_name"], "Effective": version["effective_date"], "Status": version["status"].title(), "Terms": ", ".join(terms) or "Needs review"})
        st.dataframe(rows, use_container_width=True, hide_index=True)
        for version in versions:
            if version["status"] == "active":
                try: terms = json.loads(version["policy_terms_json"] or "{}")
                except (TypeError, json.JSONDecodeError): terms = {}
                st.success(f"Active: {version['policy_number']} — {version['version_label']} · {len(terms)} extracted terms")
        with st.expander("Archive an active edition"):
            active = [v for v in versions if v["status"] == "active"]
            if active:
                opts = {f"{v['policy_number']} — {v['version_label']}": v["version_id"] for v in active}
                chosen = st.selectbox("Edition", ["Select an edition"] + list(opts))
                if chosen != "Select an edition" and st.button("Archive edition"):
                    archive_policy_version(opts[chosen], user=user); st.success("Edition archived."); st.rerun()


def main() -> None:
    st.set_page_config(page_title="Medi Gaurd AI", page_icon="MG", layout="wide", initial_sidebar_state="expanded")
    st.markdown("""
    <style>
    .block-container { max-width: 1220px; padding: 1.4rem 2rem 3rem; }
    [data-testid="stSidebar"] { border-right: 1px solid #dbe4ef; }
    [data-testid="stMetric"] { background: #f6f9fc; border: 1px solid #e1eaf2; border-radius: 12px; padding: .8rem; }
    div[data-testid="stVerticalBlock"] > div[data-testid="stVerticalBlock"] { gap: .65rem; }
    div[data-testid="stForm"] { border: 1px solid #dce7f0; border-radius: 14px; padding: 1rem; background: #fbfdff; }
    .stButton > button { border-radius: 9px; min-height: 2.6rem; font-weight: 600; }
    .auth-shell { padding: 2rem 0 .8rem; }
    .eyebrow { color: #147d92; font-weight: 700; letter-spacing: .12em; font-size: .75rem; }
    ...
    if "user" not in st.session_state:
        ...
        if pending:
            ...
            with st.form("mfa_login"):
                ...
                submitted = st.form_submit_button("Verify and sign in", type="primary")
            if submitted:
                current = get_user(pending["user_id"])
                if current and verify_totp(current, code): st.session_state.user = dict(current); st.session_state.pop("pending_mfa_user", None); st.rerun()
                else: st.error("Invalid or expired authenticator code.")
            return
        _render_login(); return
    user = st.session_state.user
    with st.sidebar:
        st.markdown("### Medi Gaurd AI")
        st.caption(f"Signed in as **{user['display_name']}**")
        st.write(f"Role: **{user['role'].title()}**")
        connected, msg = ollama_status(); (st.success if connected else st.warning)(msg)
        st.divider()
        pages = ["Dashboard", "Register claim", "Claim review"]
        if user["role"] in {"reviewer", "admin"}: pages.append("Policy management")
        page = st.radio("Workspace", pages, key="page", label_visibility="collapsed")
        if st.button("Sign out", use_container_width=True): del st.session_state.user; st.rerun()
        st.divider(); st.caption("Final determination must be confirmed by an authorized reviewer.")
    ...
    else: _render_policy_management(user)
